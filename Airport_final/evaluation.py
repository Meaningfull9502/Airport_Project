...
def compute_both_losses(model, batch, args):
    x, y = batch
    
    batch_x = x[:,:,:-1].to(args.device)
    batch_x_mark = x[:,:,-1].to(args.device)
    batch_y = y[:,:,:-1].to(args.device)
    batch_y_mark = y[:,:,-1].to(args.device)
    
    true_y = batch_y[:, -args.pred_len:]

    preds = model(batch_x, batch_x_mark, batch_y_mark)
...
def evaluation(model, dataloader, n_dataloader, args):
    ...
    with torch.no_grad():
        for _ in range(n_dataloader):
            ...
            if args.target == 'both':
                eval_res = compute_both_losses(model, batch, args)
            else:
                if args.use_covar:
                    eval_res = compute_both_losses(model, batch, args)
                else:
                    eval_res = compute_loss(model, batch, args)
