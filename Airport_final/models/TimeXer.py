...
class TimeXer_Encoder(nn.Module):
    def __init__(self, layers, norm_layer=None, projection=None):
        super(TimeXer_Encoder, self).__init__()
...
class TimeXer_EncoderLayer(nn.Module):
    def __init__(self, self_attention, cross_attention, d_model, d_ff=None, dropout=0.1, activation="relu"):
        super(TimeXer_EncoderLayer, self).__init__()
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.patch_num = int(args.seq_len // args.patch_len)
        self.n_vars = args.ndims
        self.use_covar = args.use_covar
        
        # Embedding
        self.en_embedding = EnEmbedding(self.n_vars, args.d_model, self.patch_len, args.dropout)
        self.ex_embedding = DataEmbedding_inverted(args.seq_len, args.d_model, args.dropout)
        ...
        if self.use_covar:
            self.head_nf = args.d_model * (self.patch_num + 2)
        else:
            self.head_nf = args.d_model * (self.patch_num + 1)
        self.head = FlattenHead(self.n_vars, self.head_nf, args.pred_len, head_dropout=args.dropout)
    ...
    def forward(self, x_enc, x_mark_enc=None, x_mark_dec=None):
        ...
        if self.use_covar:
            ...
            dec_out = dec_out.unsqueeze(-1).expand(-1, N, -1, -1)
